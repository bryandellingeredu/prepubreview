...
{
    public class ResubmitToOPSECAfterRevision
    {
        ...
        public class Handler : IRequestHandler<Command, Result<Unit>>
        {
            ...
            public async Task<Result<Unit>> Handle(Command request, CancellationToken cancellationToken)
            {
                ...
                var newThread = new PrePublication_Thread
                {
                    ...
                    AssignedToPersonId = publication.Threads.Where(x => x.Order == 1).First().SecurityOfficer.PersonId,
                    Type = ThreadType.OPSEC,
                };
            ...
            private async Task SendEmail(Guid threadId, int orderNumber, string commentsAsHtml)
            {
                ...
                var securityOfficer = await _userService.GetUserByPersonIdAsync(publication.Threads.Where(x => x.Order ==1).First().SecurityOfficer.PersonId);

                string title = $"A security officer review has been assigned for {publication.Title} after author revision";
                StringBuilder body = new StringBuilder();
                body.Append($"<h1> {publication.Title} has been revised and has been resubmitted for Security Officer Review </h1>");
                body.Append($"<p> <strong> Assigned To: </strong> {securityOfficer.FirstName} {securityOfficer.LastName}</p>");
                ...
                if (!string.IsNullOrEmpty(securityOfficer.EduEmail)) recipients.Add(securityOfficer.EduEmail);
                if (!string.IsNullOrEmpty(securityOfficer.ArmyEmail)) recipients.Add(securityOfficer.ArmyEmail);
